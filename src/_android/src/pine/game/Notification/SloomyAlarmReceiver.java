...
import pine.game.Sloomy.MainActivity;

public class SloomyAlarmReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		try {
			...
			String s = "data/data/" + package_name + "/localPush.p";
			...
			PineLocalPushItem localPushItem = new PineLocalPushItem();
			if (data == null) {
			...
			for (int i = 0; i < PineLocalPushItem.LOCAL_PUSH_MAX_ITEM; i++) {
				localPushItem.LoadFromBufffer(data, i * PineLocalPushItem.GetDataLength());
				...
				if (localPushItem.GetId() >= 0) {
					if (localPushItem.GetSecond() > 0) {
						...
						localPushItem.SaveToBuffer(data, i * PineLocalPushItem.GetDataLength());
					}
					if (localPushItem.GetSecond() == 0) {
