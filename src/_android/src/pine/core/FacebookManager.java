...
import static pine.core.OsFunctions.GetResourcePath;
				...
				if(success)
				{
					try 
					{
						...
						for(int i = 0; i< objects.length(); i++)
						{
							...
							friend_ids.add(Long.parseLong(obj.getString("id")));
						}
						
						for(int i = 0; i< tmp.size(); i++)
						{
							ActionFacebookAppFriendsArg fbarg = (ActionFacebookAppFriendsArg) tmp.get(i);
							fbarg._friend_ids = friend_ids;
						}
