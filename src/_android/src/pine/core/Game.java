		...
		switch(stream.ServiceCode){
							...
							public void onBegin(){
								...
								public void onDone(){
									...
									if(actstream != null){
										...
										int data_size = 4 + (8 * this._friend_ids.size());
										StreamBuffer buff = new StreamBuffer(data_size);
										
										buff.writeInt32Revert(this._friend_ids.size());
										for(int i = 0; i< this._friend_ids.size(); i++)
										{
											buff.writeInt64Revert(this._friend_ids.get(i));
										}
			...
			case ServiceCode.SendEmailIntent:{
				...
				if(stream.RequestData != null){
					...
					arg.Title = title;
					arg.Body = body;
					arg.To = new String [] {"support@pine-entertainment.com"};
					...
					((Activity) AppContext.CurrentContext).runOnUiThread(new Runnable() {
						public void run() {
							if(stream.RequestData != null){
								...
								stream.ErrorCode = PineNotificationManager.setNotification(appName, message, message, second) ? 0: 1;
								_stream_response.add(stream);
							}else {
